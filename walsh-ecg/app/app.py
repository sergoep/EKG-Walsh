...
import numpy as np
import wfdb
from walsh_ecg import WalshECGAnalyzer
from pathlib import Path

st.set_page_config(page_title="Walsh-ECG Demo", layout="wide")
st.title("Walsh–ECG: FWHT-based anomaly detection (CPU-only)")

ref_file = st.file_uploader("Upload reference .npz (from scripts/build_reference.py)", type=["npz"])
...
with col1:
    uploaded = st.file_uploader("Upload a 1-column CSV of ECG (amplitude)", type=["csv","txt"])
with col2:
...
if uploaded is not None:
    ...
    fs = st.number_input("Sampling rate of uploaded CSV", min_value=10, max_value=5000, value=256, step=1)
...
if x is not None:
    ...
    if analyzer.ref is not None:
        ...
        st.markdown(f"**Distance D(c):** `{d:.4f}`  |  **Threshold θ:** `{analyzer.ref.theta:.4f}`")
